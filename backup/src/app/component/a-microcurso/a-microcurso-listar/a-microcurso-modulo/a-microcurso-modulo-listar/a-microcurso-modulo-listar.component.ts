...
import { AMicrocursoModuloDialogoComponent } from './a-microcurso-modulo-dialogo/a-microcurso-modulo-dialogo.component';
import { MicroCursoService } from 'src/app/service/microcurso.service';

@Component({
  selector: 'app-a-microcurso-modulo-listar',
  templateUrl: './a-microcurso-modulo-listar.component.html',
  styleUrls: ['./a-microcurso-modulo-listar.component.css']
})
export class AMicrocursoModuloListarComponent implements OnInit {
  lista: Modulo[] = [];
  ...
  confirmar(id: number) {
    this.idMayor = id;
    this.dialog.open(AMicrocursoModuloDialogoComponent);
  }
  eliminar(id: number) {
    if(this.idMayor!=0){
